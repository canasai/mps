...
def print_conll_style_part(out, text, mentions, doc, part):
	doc_str = doc
	# Canasai's comment out: if "tc/ch/00/ch" in doc_str and '9' not in doc_str:
...
def print_cluster_missing_only(out, text, gold_cluster_set, covered, gold_parses, gold_heads):
	for entity in gold_cluster_set:
		printed = 0
		for mention in entity:
			if mention not in covered:
...
def print_cluster_extra_only(out, text, auto_cluster_set, covered, gold_parses, gold_heads):
	for entity in auto_cluster_set:
		printed = 0
		for mention in entity:
			if mention not in covered:
