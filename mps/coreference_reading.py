...
def read_conll_text(lines):
	...
	for line in lines:
		...
		# Canasai's comment out: if len(fields) == 0:
		if len(line) == 0:
			text.append([])
		else:
...
# Canasai's comment out: def read_conll_doc(filename, ans=None, rtext=True, rparses=True, rheads=True, rclusters=True, rner=True):
# Canasai's addition begin
